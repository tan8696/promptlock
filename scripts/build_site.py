...
people need to *see*. This renders two real reports (a clean run and a model
downgrade) plus a landing page, all as static files with no runtime.
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --panel:#1b1f26; --accent:#56cfa1;
  }
}
* { box-sizing:border-box; }
body {
  margin:0; padding:56px 24px 80px; background:var(--bg); color:var(--ink);
  font:15px/1.6 ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;
}
.wrap { max-width:760px; margin:0 auto; }
h1 { font-size:38px; margin:0 0 6px; letter-spacing:-0.02em; }
.tag { font-size:18px; color:var(--muted); margin:0 0 28px; }
...
.cta a {
  display:block; padding:13px 18px; border:1px solid var(--line);
  border-radius:10px; background:var(--panel); text-decoration:none;
  color:var(--ink); flex:1 1 240px;
}
...
<div class="cta">
  ...
  <span>Sonnet downgraded to Haiku: 6 of 50 cases regressed, 73% cheaper</span></a>
  <a href="report-pass.html"><strong>A clean report &rarr;</strong>
  <span>All 50 cases hold — what a quiet PR looks like</span></a>
</div>
<p class="note" style="margin-top:14px">Both are real output from
<code>promptlock check --html</code>. Click any row to expand a character-level
diff of baseline vs current.</p>
...
This page is a static preview of its report.
...
def main() -> int:
    ...
    for name, (variant, title) in reports.items():
        report = check(variant, baseline)
        path = os.path.join(OUT, name)
        with open(path, "w", encoding="utf-8") as f:
            f.write(htmlreport.render(report, title))
        print(f"  {path:<26} {report['summary']}")

    index = os.path.join(OUT, "index.html")
    with open(index, "w", encoding="utf-8") as f:
        f.write(PAGE)
    print(f"  {index:<26} landing page")
    return 0
